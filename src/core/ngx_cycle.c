...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    // 循环 cycle 的 modules 数组，并调用每一个 module 的 create_conf 函数
    ...
    // 循环 cycle 的 module 数组，并调用每一个 module 的 init_conf 函数
    // 这里只初始化 module 类型为 NGX_CORE_MODULE 类型的 module
    // nginx.c 所处的 core module，也是 NGX_CORE_MODULE 类型的 module
    // 所以大部分主模块的初始化工作都在这里进行了
    for (i = 0; cycle->modules[i]; i++) {
        if (cycle->modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }

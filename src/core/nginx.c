...
// 下面的三个全局变量都是在启动时传入参数进行初始化的
static ngx_uint_t   ngx_show_help; // 是否展示帮助 -h
static ngx_uint_t   ngx_show_version; // 是否展示版本信息
static ngx_uint_t   ngx_show_configure; // 是否展示选项 -V
...
main(int argc, char *const *argv)
{
    ...
    // ngx_get_option 的时候会修改这个全局变量
    if (ngx_show_version) {
        // 抽成函数了，老版本这个函数的代码都是写在 main 里的哈哈
    ...
    // 把启动程序时的 argv, argc 赋值给全局变量，之后在各子进程/线程中能读得到
    if (ngx_save_argv(&init_cycle, argc, argv) != NGX_OK) {
        return 1;
    }

    // 你看他是 process_options，实际上做的都是简单的工作，没有实际去读取 config 内容
    // 只是修改了 config 之类的参数，拿出完整的 config 文件路径
    ...
    // 预初始化 module，不过实际上没做什么事情。。
    // 只是给 module 们写了个 index 和 name
    // preinit_modules 老版本也是直接写在 main 函数里的。。
...
ngx_process_options(ngx_cycle_t *cycle)
{
    ...
    if (ngx_conf_file) {
        ...
        // -c 会设置 ngx_conf_file，没有的话就读默认的路径 conf/nginx.conf
        ngx_str_set(&cycle->conf_file, NGX_CONF_PATH);
    }
    
    // 判断 -c 后面提供的文件名是不是一个绝对路径
    // 如果不是的话，获取这个文件完整的绝对路径
